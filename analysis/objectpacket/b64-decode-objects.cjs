...
const content = JSON.stringify(decoded)
const objects = []
// More flexible pattern - matches objects with various whitespace
const regex =
  /\[\s*\[\s*(\d+)\s*\]\s*,\s*(\d+)\s*,\s*0\s*,\s*2\s*,\s*0\s*,\s*(\d+)\s*,\s*0\s*,\s*0\s*,\s*\[\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]\s*,\s*\[\s*(\d+)\s*\]\s*,\s*(\d+)\s*,\s*(true|false)\s*\]/g

let match
while ((match = regex.exec(content)) !== null) {
  objects.push({
    objectId: parseInt(match[1]),
    staticId: parseInt(match[2]),
    level: parseInt(match[3]),
    kingdom: parseInt(match[4]),
    x: parseInt(match[5]),
    y: parseInt(match[6]),
    unk6: parseInt(match[7]),
    extra: parseInt(match[8]),
    unk7: match[9] === 'true'
  })
}
